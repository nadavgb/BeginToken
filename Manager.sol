...
/**
 * The Manager contract is the point of contact between the Begin ecosystem and the
 * surrounding world. It manages the Issuance and Redemption of BGN, a decentralized stablecoin
 * backed by a basket of tokens.
 *
 * The Manager also implements a Proposal system to handle administration of changes to the
 * backing of BGN. Anyone can propose a change to the backing.  Once the `owner` approves the
 ...
 *
 * There are two different ways to propose changes to the backing of BGN:
 * - proposeSwap()
 * - proposeWeights()
 *
 * In both cases, tokens are exchanged with the Vault and a new BGN backing is set. You can
 ...
 * - 1 BGN: 1 Begin
 * - 1 qBGN: 1 quantum of Begin.
 *      (BGN & qBGN are convertible by .mul(10**reserve.decimals() qBGN/BGN))
...
contract Manager is Ownable {
    ...
    function isFullyCollateralized() public view returns(bool) {
        uint256 scaleFactor = WEIGHT_SCALE.mul(uint256(10) ** trustedBGN.decimals());
        // scaleFactor unit: aqToken/qToken * qBGN/BGN

        for (uint256 i = 0; i < trustedBasket.size(); i++) {

            address trustedToken = trustedBasket.tokens(i);
            uint256 weight = trustedBasket.weights(trustedToken); // unit: aqToken/BGN
            ...
            if (trustedBGN.totalSupply().mul(weight) > balance.mul(scaleFactor)) {
                // checking units: [qBGN] * [aqToken/BGN] == [qToken] * [aqToken/qToken * qBGN/BGN]
                return false;
            }
        }
        return true;
    ...
    /// Get amounts of basket tokens required to issue an amount of BGN.
    /// The returned array will be in the same order as the current basket.tokens.
    /// return unit: qToken[]
    function toIssue(uint256 bgnAmount) public view returns (uint256[] memory) {
        // bgnAmount unit: qBGN.
        uint256[] memory amounts = new uint256[](trustedBasket.size());

        uint256 feeRate = uint256(seigniorage.add(BPS_FACTOR));
        // feeRate unit: BPS
        uint256 effectiveAmount = bgnAmount.mul(feeRate).div(BPS_FACTOR);
        // effectiveAmount unit: qBGN == qBGN*BPS/BPS
        ...
        for (uint256 i = 0; i < trustedBasket.size(); i++) {
            ...
            // unit: qToken = _weighted(qBGN, aqToken/BGN, _)
        }

        return amounts; // unit: qToken[]
    }

    /// Get amounts of basket tokens that would be sent upon redeeming an amount of BGN.
    /// The returned array will be in the same order as the current basket.tokens.
    /// return unit: qToken[]
    function toRedeem(uint256 bgnAmount) public view returns (uint256[] memory) {
        // bgnAmount unit: qBGN
        uint256[] memory amounts = new uint256[](trustedBasket.size());
    ...
    /// Handles issuance.
    /// bgnAmount unit: qBGN
    function issue(uint256 bgnAmount) external
        issuanceNotPaused
        notEmergency
        vaultCollateralized
    {
        require(bgnAmount > 0, "cannot issue zero BGN");
        require(trustedBasket.size() > 0, "basket cannot be empty");

        // Accept collateral tokens.
        uint256[] memory amounts = toIssue(bgnAmount); // unit: qToken[]
        for (uint256 i = 0; i < trustedBasket.size(); i++) {
        ...
        // Compensate with BGN.
        trustedBGN.mint(_msgSender(), bgnAmount);
        // unit check for bgnAmount: qBGN.

        emit Issuance(_msgSender(), bgnAmount);
    }

    /// Handles redemption.
    /// bgnAmount unit: qBGN
    function redeem(uint256 bgnAmount) external notEmergency vaultCollateralized {
        require(bgnAmount > 0, "cannot redeem 0 BGN");
        require(trustedBasket.size() > 0, "basket cannot be empty");

        // Burn BGN tokens.
        trustedBGN.burnFrom(_msgSender(), bgnAmount);
        // unit check: bgnAmount is qBGN.

        // Compensate with collateral tokens.
        uint256[] memory amounts = toRedeem(bgnAmount); // unit: qToken[]
        for (uint256 i = 0; i < trustedBasket.size(); i++) {
            trustedVault.withdrawTo(trustedBasket.tokens(i), amounts[i], _msgSender());
            // unit check for amounts[i]: qToken.
        }

        emit Redemption(_msgSender(), bgnAmount);
    }
     ...
     *    total supply of BGN **at the time when the proposal is executed**.
     *
     ...
     * composition towards some known target of Begin's management while maintaining full
     * backing; the expected behavior of Begin's management is to accept only such proposals,
     ...
     * proposer may change between proposition and execution. If the supply of BGN rises or falls,
    ...
    function executeProposal(uint256 id) external onlyOperator notEmergency vaultCollateralized {
        ...
        trustedBasket = trustedProposals[id].complete(trustedBGN, trustedOldBasket);
    ...
    function _executeBasketShift(
        uint256 oldWeight, // unit: aqTokens/BGN
        uint256 newWeight, // unit: aqTokens/BGN
    ...
    // When you perform a weighting of some amount of BGN, it will involve a division, and
    // precision will be lost. When it rounds, do you want to round UP or DOWN? Be maximally
    // conservative.
    enum RoundingMode {UP, DOWN}

    /// From a weighting of BGN (e.g., a basket weight) and an amount of BGN,
    /// compute the amount of the weighted token that matches that amount of BGN.
    function _weighted(
        uint256 amount, // unit: qBGN
        uint256 weight, // unit: aqToken/BGN
        RoundingMode rnd
    ) internal view returns(uint256) // return unit: qTokens
    {
        uint256 scaleFactor = WEIGHT_SCALE.mul(uint256(10)**(trustedBGN.decimals()));
        // scaleFactor unit: aqTokens/qTokens * qBGN/BGN
        uint256 shiftedWeight = amount.mul(weight);
        // shiftedWeight unit: qBGN/BGN * aqTokens
        ...
        if (rnd == RoundingMode.DOWN || shiftedWeight.mod(scaleFactor) == 0) {
            ...
            // return unit: qTokens == qBGN/BGN * aqTokens * (qTokens/aqTokens * BGN/qBGN)
        }
