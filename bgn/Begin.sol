...
import "./BeginEternalStorage.sol";
 ...
 *
 * Non-constant-sized data is held in BeginEternalStorage, to facilitate potential future upgrades.
 */
contract Begin is IERC20, Ownable {
    ...
    BeginEternalStorage internal trustedData;
    ...
    event EternalStorageTransferred(address indexed newBeginAddress);
    ...
    string public constant name = "Begin";
    ...
    constructor() public {
        ...
        trustedData = BeginEternalStorage(address(0));
    }
    ...
    /// Make a different address the EternalStorage contract's BeginAddress.
    /// This will break this contract, so only do it if you're
    /// abandoning this contract, e.g., for an upgrade.
    function transferEternalStorage(address newBeginAddress) external onlyOwner isPaused {
        require(newBeginAddress != address(0), "zero address");
        emit EternalStorageTransferred(newBeginAddress);
        trustedData.updateBeginAddress(newBeginAddress);
    }
    ...
    /// @return how many attoBGN are held by `holder`.
    function balanceOf(address holder) external view returns (uint256) {
        return trustedData.balance(holder);
    }

    /// @return how many attoBGN `holder` has allowed `spender` to control.
    function allowance(address holder, address spender) external view returns (uint256) {
        return trustedData.allowed(holder, spender);
    }

    /// Transfer `value` attoBGN from `msg.sender` to `to`.
...
// ===========================  Upgradeability   =====================================
    ...
    /// Accept upgrade from previous BGN instance. Can only be called once. 
    function acceptUpgrade(address previousImplementation) external onlyOwner {
        ...
        Begin previous = Begin(previousImplementation);
        trustedData = BeginEternalStorage(previous.getEternalStorageAddress());
